...
def _preprocess_true_boxes(true_boxes, anchors, in_shape, num_classes, max_boxes, label_smooth,
    ...
    # print(true_boxes, input_shape.shape)
    ...
    if len(wh) != 0:
        ...
        boxes_max = wh / 2.
        boxes_min = -boxes_max
        # print(f'bmin:{boxes_min}')
        # print(f'wh:{wh}')
        intersect_min = np.maximum(boxes_min, anchors_min)
        # print(f'imin:{intersect_min}')
        intersect_max = np.minimum(boxes_max, anchors_max)
        intersect_wh = np.maximum(intersect_max - intersect_min, 0.)
        # print(f'iwh:{intersect_wh}')
        ...
        for t, n in enumerate(best_anchor):
            ...
            for l in range(num_layers):
                ...
                if n in anchor_mask[l]:
                    ...
                    # print(f'trbox:{true_boxes}')
                    k = anchor_mask[l].index(n)
                    c = true_boxes[t, 4].astype('int32')
                    # print(f'i,j,k,c,t,n,l:{i,j,k,c,t,n,l}')
                    # print(f'yt:{y_true[l].shape}')
...
def _data_aug(image, box, jitter, hue, sat, val, image_input_size, max_boxes,
    ...
    # print(f'image_w, image_h:{image.size},input_h, input_w:{image_input_size}')
    # print(box)
    ...
    if _rand() < 0.0:
        ...
        dx, dy, nw, nh = candidate
        # print(f'candidate:{candidate},boxdata:{box_data}')
        ...
        # print(f'image, ratio, pad:{image.shape, ratio, pad}')
    ...
    image_data = image_data.astype(np.float32)
    # print(f'imageshape,{image.shape}，box_data:{box_data}')
...
class MultiScaleTrans:
    ...
    def generate_seed_list(self, init_seed=1234, seed_num=int(1e6), seed_range=(1, 1000)):
        ...
        return seed_list
    #list[numpy_image] , list
    def __call__(self, imgs, annos, x1, x2, x3, x4, x5, x6, batchInfo):
        # print(imgs[0].shape, annos[0].shape, x1, x2, x3, x4, x5, x6, batchInfo)
        epoch_num = batchInfo.get_epoch_num()
        size_idx = int(batchInfo.get_batch_num() / self.resize_rate)
        # print(f'epochnum:{batchInfo.get_batch_num()},sizeidx:{size_idx},resizeratr:{self.resize_rate}')
        seed_key = self.seed_list[(epoch_num * self.resize_count_num + size_idx) % self.seed_num]
        # print(f'resize_count_num:{self.resize_count_num },seed_num:{self.seed_num},seed_key:{seed_key}')
        ret_imgs = []
        ret_annos = []
        ...
        if self.size_dict.get(seed_key, None) is None:
            # print(f'self.size_dict.get(seed_key, None):{self.size_dict.get(seed_key, None)}')
