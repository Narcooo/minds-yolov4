...
def parse_args():
    ...
    parser.add_argument('--per_batch_size', default=4, type=int, help='Batch size for Training. Default: 32.')

    # network related
    parser.add_argument('--pretrained_backbone', default='/data/cspdarknet53_ascend_v120_imagenet2012_official_cv_bs64_top1acc7854_top5acc9428.ckpt', type=str,
    ...
    parser.add_argument('--lr', default=0.004, type=float, help='Learning rate. Default: 0.001')
    ...
    # loss related
    parser.add_argument('--loss_scale', type=int, default=1, help='Static loss scale. Default: 1024')
    ...
    parser.add_argument('--resize_rate', default=None,
    ...
    args.data_root = os.path.join(args.data_dir, 'images_clip_det')
    args.annFile = os.path.join(args.data_dir, 'clip_instances_train.json')
...
def train():
    ...
    if is_gpu:
        ...
        loss_scale = FixedLossScaleManager(loss_scale_value, drop_overflow_update=True)
    ...
    for i, data in enumerate(data_loader):
        ...
        if isinstance(loss, tuple):
            loss_meter.update(loss[0].asnumpy())
        else:
